 ...
 * 时间复杂度分析；
 * 无论是最好情况还是最差情况，堆排序的时间复杂度都为O( N*LOG(N) ).
 * 堆排序不是稳定的排序，而且由于初始构建堆所需的比较次数较多，不适于待排序序列个数较少的情况。
 ******************************************************************************/
...
void HeapAdjust(SqList *L, int s, int m){
   ...
   for(int j = 2*s; j < m; j *= 2){//沿着关键字较大的孩子结点向下筛选,注意 j 的变化。
     ...
     L->r[s] = L->r[j]; //若temp不当前最大的，把较大的孩子插在s上，从这个孩子开始再往下筛选。(j 赋给s后，留下一个空位，作为下一个s)
     s = j;
   }
   L->r[s] = temp;//插入
 }
void HeapSort(SqList *L){
   ...
   /*******************************************************************************
 * 归并排序
 * 算法思想：假设初始序列含有n个记录，则可以看成是n个有序的子序列，每个子序列的长度为1
 * 然后两两归并，得到[n/2]个长度为2或者1的有序子序列；再两两归并......如此重复，直到得到一个长度
 * 为n的有序序列为止。这种排序方法称为2路归并排序。观察流程图，会发现其特别像一个倒置的完全二叉树，涉及到
 * 平衡二叉树的算法，效率一般不低。
 * 归并排序的时间复杂度:归并排序是一种非常稳定的排序，最好、最坏和平均时间复杂度都为O(nlogn)。
 * 总体来说，归并排序是占用内存较高，但是比较稳定高效的算法。
 ******************************************************************************/
